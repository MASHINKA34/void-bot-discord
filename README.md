...
## Перенос настроек между хостами

Команды владельца бота:

| Команда | Действие |
| --- | --- |
| `!backup` | прислать `storage/guilds.json` файлом в чат |
| `!restore` | загрузить настройки из прикреплённого к сообщению файла |

`!restore` полностью заменяет текущие настройки содержимым файла.
